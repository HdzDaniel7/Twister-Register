...
import {
  MAT_DEFAULT, RESTRAINT_DEFAULT, ROT_STIFF_FAC, pinFit, restrain, restrainedFree,
...
function solveContacts(model: Model, pins: Pin[], peds: Pedestal[], sec: Section,
  ...
  /* --- rigidez de cada incógnita ----------------------------------------
     ...
     fibra extrema. El rodado cede más que el ángulo por `ROT_STIFF_FAC`, que es
     el MISMO número que reparte el amarre en engine/pins.ts —por eso se importa
     en vez de teclearlo—; el porqué, las cifras de cuánto se aparta de la
     torsión real de la sección y qué respuesta lo va a cambiar están junto a la
     constante.
  ...
  for (let i = 0; i < nb; i++) {
    ...
    if (doRot) K.push(ROT_STIFF_FAC * E0 * Ia / span[i] * D2R * D2R);
  }
