...
function pedRow(M: Model, i: number, f: PedFit | null, vano: number): string {
...
export function paneFixture(M: Model): string {
  ...
  const rows = ST.fixture.map((_, i) => pedRow(M, i, fits[i], vanos[i])).join('');
  ...
  return `<div class="pane on"><div class="grp">
    ...
    ${ST.fixture.length ? `<div class="tw"><table class="marks"><thead><tr>
      ...
      <th>${T('pedGap')}</th><th>${T('pedSpan')}</th><th></th></tr></thead>
    ...
    </div>` : `<div class="hintline">${T('fixEmpty')}</div>`}
